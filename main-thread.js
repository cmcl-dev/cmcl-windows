...
return download_multi(u,path.join(__minecraft,`./libraries/${p}`),true);
};
...
const fs=require("fs")
	...
	dirs.mkdirsSync(path.join(__minecraft,`./versions/${forge_mc_ver}-Forge${forge_ver}-tmp/uncompressed`), { recursive: true });
	download_multi(forge_url,(path.join(__minecraft,`./versions/${forge_mc_ver}-Forge${forge_ver}-tmp/${forge_mc_ver}-Forge${forge_ver}-tmp.installer.jar`)),true,
	async function(){
	$("#forge_downloading").value=50;
	console.log("Start Compressing")
	await compressing.zip.uncompress(path.join(__minecraft,`./versions/${forge_mc_ver}-Forge${forge_ver}-tmp/${forge_mc_ver}-Forge${forge_ver}-tmp.installer.jar`),path.join(__minecraft,`./versions/${forge_mc_ver}-Forge${forge_ver}-tmp/uncompressed`))
	let forge_install_json=JSON.parse(fs.readFileSync(path.join(__minecraft,`./versions/${forge_mc_ver}-Forge${forge_ver}-tmp/uncompressed/install_profile.json`)));
	let forge_current_name=(forge_install_json.install)?(forge_install_json.install.target):(forge_install_json.version);
	dirs.mkdirsSync(path.join(__minecraft,`./versions/${forge_current_name}`), { recursive: true });
	if(forge_install_json.versionInfo)
	fs.writeFileSync(path.join(__minecraft,`./versions/${forge_current_name}/${forge_current_name}.json`),JSON.stringify(forge_install_json.versionInfo));
	else if(forge_install_json.json)
	fs.writeFileSync(path.join(__minecraft,`./versions/${forge_current_name}/${forge_current_name}.json`),fs.readFileSync(path.join(__minecraft,`./versions/${forge_mc_ver}-Forge${forge_ver}-tmp/uncompressed/version.json`)));
	
	let libraries=JSON.parse(fs.readFileSync(path.join(__minecraft,`./versions/${forge_current_name}/${forge_current_name}.json`))).libraries;
...
download_multi(obj["dataset"]["url"],path.join(__minecraft,`./libraries/${obj["dataset"]["path"]}`),true,
async function(){
	obj["download_progress"].value=100;
	if(obj["dataset"]["path"].indexOf("natives")!==-1)await jar_natives(path.join(__minecraft,`./versions/${ver_arr[d_num]}/${ver_arr[d_num]}-natives`),[path.join(__minecraft,`./libraries/${obj["dataset"]["path"]}`)]);setTimeout(function(){obj.parentNode.parentNode.hidden=true;},1000);
	},function(cgh){
	obj["download_progress"].value=cgh;
});
}
function re_download_assets(obj){
	const fs=require("fs")
	console.log(obj["download_progress"])
download_multi(obj["dataset"]["url"],path.join(__minecraft,`./assets/objects/${obj["dataset"]["hash"].slice(0,2)}/${obj["dataset"]["hash"]}`),true,async function(){obj["download_progress"].value=100;setTimeout(function(){obj.parentNode.parentNode.hidden=true;},1000);},function(cgh){obj["download_progress"].value=cgh;});
}
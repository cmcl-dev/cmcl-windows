...
function parse_mc_args_v1(version_json, options) {
    ...
    if (version_json.minecraftArguments) {
        ...
        let jvm = Array.from(version_json.arguments.jvm),
            game = Array.from(version_json.arguments.game);
        if (!game) game = [];
        if (!jvm) jvm = [];
        let mcarg = [...jvm, ...game],
            new_arg = [],
...
function launched_v1(_options) {
    ...
    */
    ...
    let mcargs = parse_mc_args(version_json, options),
        ____jvm = mcargs.jvm,
        ____games = mcargs.game;
...
function launched_v2(_options) {
    ...
    */
    ...
    function parse_mc_args_v2() {
        let assets_id;
        do {
            let v1 = version_json.assetIndex;
            assets_id = v1 ? v1.id : version_json.assets;
        } while (0);
        const crypto = require('crypto');
        let m_ = crypto.createHash('md5').update(options.settings.username, 'utf8').digest('hex');
        ...
        if (version_json.minecraftArguments) {
            ...
            return { game: new_mcarg.split(" "), jvm: [`-Djava.library.path=\"${natives}\"`,"-cp", `"${options.classpath}"`] };
        } else {
            let jvm = Array.from(version_json.arguments.jvm),
                game = Array.from(version_json.arguments.game);
            if (!game) game = [];
    ...
    let mcargs = parse_mc_args_v2(version_json, options),
        ____jvm = mcargs.jvm,
        ____games = mcargs.game;

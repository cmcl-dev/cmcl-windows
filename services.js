...
let __main = function () {
	...
	function push_succeed() {
		postMessage(JSON.stringify({ type: "succeed" }));
	}
	/*
	{'jsonrpc': '2.0', 'id': 'qwer',
	'method': 'aria2.addUri',
	...
	*/
	...
	async function download_aria2RPC(_hash, u, p, async_f, cb, ac_cb, failed_cb) {

	}
	async function download_aria2Command(_hash, u, p, async_f, cb, ac_cb, failed_cb) {

	}
	async function universal_download(_hash, u, p, async_f, cb, ac_cb, failed_cb) {
		if (!localStorage["--flag-download"]) localStorage["--flag-download"] = "multi-request";
		switch (localStorage["--flag-download"]) {
			case "multi-request": {
				return download_multi_p(_hash, u, p, async_f, cb, ac_cb, failed_cb);
				break;
			}
			case "multi-file": {
				return download_multi(_hash, u, p, async_f, cb, ac_cb, failed_cb);
				break;
			}
			case "aria2-rpc": {
				return download_aria2RPC(_hash, u, p, async_f, cb, ac_cb, failed_cb);
				break;
			}
			case "aria2-command": {
				return download_aria2Command(_hash, u, p, async_f, cb, ac_cb, failed_cb);
				break;
			}
		}
	}
	...
	let d = async function (num) {
		...
		for (var s = 0; s < lib_len; s++) {
			...
			if (o.downloads.classifiers && (!rule_parsing(o.rules))) {
				if (o.downloads.classifiers["natives-windows"]) {
...
onmessage = async function (e) {
	...
	__main();
	if (obj.main_exec) { functions.d(obj.d_num); }
}